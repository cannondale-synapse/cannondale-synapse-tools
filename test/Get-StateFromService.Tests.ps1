...
InModuleScope azure.synapse.tools {
    ...
    Describe 'Get-StateFromService' {
        BeforeAll {
            $storageKey = [Convert]::ToBase64String([byte[]](1..64))
            $mockStorageContext = New-AzStorageContext -StorageAccountName 'storage1' -StorageAccountKey $storageKey
            Mock -CommandName New-AzStorageContext -MockWith {
                $mockStorageContext
            }
        }
